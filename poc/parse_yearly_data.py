...
def process_folder(folder: Path):
    ...
    # Try read_only=True first (fast); fall back to read_only=False if openpyxl
    # chokes on malformed autofilter XML (common in some 2022 templates).
    ...
    try:
        ...
        print(f"    [ERR] Cannot open .xlsm (read_only): {e}")
        return None

    def _parse_with_fallback(parse_fn, wb_ref, label):
        """Run parse_fn(wb); on ValueError (openpyxl filter quirk) reload w/o read_only."""
        try:
            return parse_fn(wb_ref), wb_ref
        except ValueError as e:
            msg = str(e)
            if "wildcard" in msg or "numerical" in msg:
                print(f"    [WARN] {label}: openpyxl read_only quirk, retrying full load…")
                try:
                    wb_ref.close()
                except Exception:
                    pass
                wb2 = _load(read_only=False)
                return parse_fn(wb2), wb2
            raise

    # Metadata (Accueil sheet — extended)
    try:
        meta, wb = _parse_with_fallback(parse_accueil, wb, "Accueil parse")
    except Exception as e:
    ...
    try:
        chiffrage, wb = _parse_with_fallback(parse_chiffrage, wb, "Chiffrage parse")
    except Exception as e:
